# Blender 2.77 Cycles Material Script 
# Original Script by Ben (276)
# Revisions by Nick Ritter (277)
#    Original script called keys for each data.materials,
#    but data.materials isn't an array, so I changed the code to
#    iterate through all the data.materials instead, regardless of object.
#    Also, it'll load the images into your .blend file if they're not already loaded.
#    You're welcome.
...
#   |- tex (Folder)
#      |- a lot of png files
#   |- minecraft.obj    <---can be named whatever
#   |- minecraft.mtl    <---can be named whatever

# - Import minecraft.obj into Blender
# - Switch to "Scripting"
# - Open this script
# - Press "Run Script"
#
...
def doIt():
    ...
    for mat in bpy.data.materials:
            ...
            print ('Set Material for: '+mat.name)
            mat.use_nodes = True
            ...
            if (bpy.data.images[mat.name+'.png']):
                continue
            else:
                try:
                    bpy.ops.image.open(filepath = '//tex\\'+texName+'.png')
                except:
                    print('ERROR opening file '+texName+'.png')
            ...
            # specials
            if (mat.name == 'armor_enchanted'):
                armor_enchanted(nodes, links);
            else:
                if texImageFound == False:
                    print ('TexImage for '+mat.name+' not found!')
                else:
                    ...
                    try:
                        nMainData = mainShader[mat.name](nodes, links)
                    except KeyError:
                    ...
                    if (
                        (mat.name == 'sea_lantern') or 
                        (mat.name == 'torch_flame') or 
                        (mat.name == 'fire') or 
                        ((mat.name.startswith( 'pumpkin_') and mat.name.endswith( '_lit')))
                    ):
                        mat.cycles.sample_as_light = True
                        ...
                        if (mat.name.startswith( 'pumpkin_')):
                    ...
                    if ((mat.name.startswith( 'armor_')) and 
                        (mat.name != 'armor_stand') and
                        (mat.name != 'armor_enchanted')
                    ):
                        if (mat.name.startswith('armor_leather')):
